...
class Chexpert_VQA_Trainer(ChexpertTrainerBase):
    def __init__(self, transform, batch_size, collate_batch_fn, num_workers, tokenizer):
        ...
        for i in range(1, len(CHEXPERT_LABELS)):
            ...
            pos_dataset = ChexpertVQADataset(
                self.image_paths, transform, self.orientations, self.genders, self.labels,
                question=i, answer=pos_answer, indices=pos_indices, infinite=True)
            ...
            neg_dataset = ChexpertVQADataset(
                self.image_paths, transform, self.orientations, self.genders, self.labels,
                question=i, answer=neg_answer, indices=neg_indices, infinite=True)
