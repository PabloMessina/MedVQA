...
from medvqa.utils.torch_utils import freeze_model_parts, log_module_trainability
...
from medvqa.utils.common import DictWithDefault, inspect_available_ram
from medvqa.metrics import (
    ...
    attach_condition_aware_bbox_soft_dice,
...
from medvqa.utils.files_utils import find_existing_checkpoint_folder_paths, get_checkpoint_folder_path, load_config_yaml
...
def parse_args(args=None):
    # --- Step 1: Parse the config file argument first ---
    conf_parser = argparse.ArgumentParser(add_help=False)
    conf_parser.add_argument('--config_filepath', help='Path to YAML config file')
    known_args, remaining_args = conf_parser.parse_known_args(args)
    ...
    # --- Step 3: Define the main parser ---
    parser = argparse.ArgumentParser(parents=[conf_parser]) 
    
    # --- Required arguments (Moved to manual check) ---
    # NOTE: 'required=True' is REMOVED. We check these manually at the end.
    parser.add_argument('--experiment_name', type=str, help='Name of the experiment')
    parser.add_argument('--epochs', type=int, help='Number of epochs')
    parser.add_argument('--batches_per_epoch', type=int, help='Number of batches per epoch')
    parser.add_argument('--max_images_per_batch', type=int, help='Max number of images per batch')
    parser.add_argument('--max_phrases_per_batch', type=int, help='Max number of phrases per batch')
    parser.add_argument('--max_phrases_per_image', type=int, help='Max number of phrases per image')
    parser.add_argument('--val_batch_size_factor', type=float, help='Factor for test dataloader')

    # --- Other arguments ---
    parser.add_argument('--checkpoint_folder_path', type=str, default=None,
                        help='Path to folder with checkpoint to resume training from')
    ...
    parser.add_argument('--freeze_config', type=str, default=None)
    ...
    parser.add_argument('--chest_imagenome_train_max_size', type=int, default=None)
    parser.add_argument('--chest_imagenome_val_max_size', type=int, default=None)
    parser.add_argument('--padchest_gr_val_max_size', type=int, default=None)
    parser.add_argument('--vinbig_val_max_size', type=int, default=None)
    ...
    parser.add_argument('--use_cnr_metric', action='store_true',
                        help='Use CNR metric instead of IoU for bounding box metrics')
    parser.add_argument('--use_soft_dice_metric', action='store_true',
                        help='Use soft dice metric instead of IoU for bounding box metrics')
    ...
    # Wandb arguments
    parser.add_argument('--wandb_config', type=dict, default=None)

    # --- Step 4: Apply YAML Defaults ---
    # This ensures YAML values overwrite the 'default=None' from add_argument
    if defaults:
    ...
    # --- Manual Validation for "Required" Args ---
    # Since we removed required=True, we check if they are None here.
    required_args = [
        'experiment_name',
        'epochs', 
        'batches_per_epoch', 
    ...
    if missing_args:
        parser.error(f"The following arguments are required (either in YAML or CLI): {', '.join(missing_args)}")
...
_METRIC_WEIGHTS['vbg_prc_auc'] = 5.0 # Assign more weight to classification metrics for VinBig
...
def train_model(
    ...
    use_cnr_metric = experiment_kwargs.get('use_cnr_metric', False)
    use_soft_dice_metric = experiment_kwargs.get('use_soft_dice_metric', False)
    experiment_name = experiment_kwargs['experiment_name']

    if skip_nms:
        assert use_cnr_metric or use_soft_dice_metric # if we skip NMS, we need to compute CNR or soft dice
    ...
    if use_mimiccxr_facts_for_train:
        if experiment_kwargs['mimiccxr_facts_weight'] == 0:
    ...
    if use_chest_imagenome_for_train:
        if (experiment_kwargs['chest_imagenome_pg_weight'] == 0 or
            experiment_kwargs['chest_imagenome_alg_weight'] == 0):
    ...
    if use_mscxr_for_train:
        if experiment_kwargs['mscxr_weight'] == 0:
    ...
    if use_cxrlt2024_challenge_split:
        if experiment_kwargs['cxrlt2024_weight'] == 0:
    ...
    if use_vinbig_for_train:
        if experiment_kwargs['vinbig_weight'] == 0:
    ...
    if use_padchest_gr_for_train:
        if experiment_kwargs['padchest_gr_weight'] == 0:
    ...
    if use_chexlocalize_for_train:
        if experiment_kwargs['chexlocalize_weight'] == 0:
    ...
    if use_chexpert_for_train:
        if experiment_kwargs['chexpert_weight'] == 0:
    ...
    if use_iuxray_for_train:
        if experiment_kwargs['iuxray_weight'] == 0:
    ...
    if use_mimiccxr_facts_for_train:
        ...
        _train_weights.append(experiment_kwargs['mimiccxr_facts_weight'])
    ...
    if use_mscxr_for_train:
        ...
        _train_weights.append(experiment_kwargs['mscxr_weight'])
    ...
    if use_chest_imagenome_for_train:
        ...
        _train_weights.append(experiment_kwargs['chest_imagenome_alg_weight'])
        ...
        _train_weights.append(experiment_kwargs['chest_imagenome_pg_weight'])
    ...
    if use_cxrlt2024_challenge_split or use_cxrlt2024_custom_labels:
        ...
        wo *= experiment_kwargs['cxrlt2024_weight'] # scale weights
        wc *= experiment_kwargs['cxrlt2024_weight'] # scale weights
        if use_cxrlt2024_official_labels:
    ...
    if use_vinbig_for_train:
        ...
        if vinbig_task_mode == VinBigPhraseTaskMode.CLASSIFICATION.value:
            _train_weights.append(experiment_kwargs['vinbig_weight'])
            _train_dataloaders.append(vinbig_trainer.train_classification_dataloader)
            logger.info(f'len(vinbig_trainer.train_classification_dataloader) = {len(vinbig_trainer.train_classification_dataloader)}')
        elif vinbig_task_mode == VinBigPhraseTaskMode.GROUNDING.value:
            _train_weights.append(experiment_kwargs['vinbig_weight'])
            _train_dataloaders.append(vinbig_trainer.train_grounding_dataloader)
            logger.info(f'len(vinbig_trainer.train_grounding_dataloader) = {len(vinbig_trainer.train_grounding_dataloader)}')
        elif vinbig_task_mode == VinBigPhraseTaskMode.CLASSIFICATION_AND_GROUNDING.value:
            _train_weights.append(0.5 * experiment_kwargs['vinbig_weight'])
            _train_dataloaders.append(vinbig_trainer.train_classification_dataloader)
            _train_weights.append(0.5 * experiment_kwargs['vinbig_weight'])
            _train_dataloaders.append(vinbig_trainer.train_grounding_dataloader)
            logger.info(f'len(vinbig_trainer.train_classification_dataloader) = {len(vinbig_trainer.train_classification_dataloader)}')
    ...
    if use_vinbig_for_test:
        if vinbig_task_mode == VinBigPhraseTaskMode.CLASSIFICATION.value:
            _val_dataloaders.append(vinbig_trainer.val_classification_dataloader)
            logger.info(f'len(vinbig_trainer.val_classification_dataloader) = {len(vinbig_trainer.val_classification_dataloader)}')
        elif vinbig_task_mode == VinBigPhraseTaskMode.GROUNDING.value:
            _val_dataloaders.append(vinbig_trainer.val_grounding_dataloader)
            logger.info(f'len(vinbig_trainer.val_grounding_dataloader) = {len(vinbig_trainer.val_grounding_dataloader)}')
        elif vinbig_task_mode == VinBigPhraseTaskMode.CLASSIFICATION_AND_GROUNDING.value:
            _val_dataloaders.append(vinbig_trainer.val_classification_dataloader)
            _val_dataloaders.append(vinbig_trainer.val_grounding_dataloader)
            logger.info(f'len(vinbig_trainer.val_classification_dataloader) = {len(vinbig_trainer.val_classification_dataloader)}')
    ...
    if use_padchest_gr_for_train:
        ...
        _train_weights.append(experiment_kwargs['padchest_gr_weight'])
    ...
    if use_chexlocalize_for_train:
        ...
        _train_weights.append(experiment_kwargs['chexlocalize_weight'])
    ...
    if use_chexpert_for_train:
        ...
        _train_weights.append(experiment_kwargs['chexpert_weight'])
    ...
    if use_iuxray_for_train:
        ...
        _train_weights.append(experiment_kwargs['iuxray_weight'])
    ...
    freeze_config = model_kwargs.get('freeze_config', None)
    if freeze_config is not None:
        freeze_config = load_config_yaml(freeze_config)
        freeze_model_parts(model, freeze_config)
    ...
    if use_chest_imagenome_for_train or use_chest_imagenome_for_val:
        ...
        def _cond_func(x):
            return x['dataset_name'] == 'chest-imagenome-pg'
        if in_train:
        ...
        def _cond_func(x):
            return x['dataset_name'] == 'chest-imagenome-alg'
        if in_train:
        ...
        if in_val:
            if skip_nms:
                ...
                if use_soft_dice_metric:
                    attach_condition_aware_bbox_soft_dice(validator_engine, field_names=['pred_bbox_probs', 'bbox_coords', 'bbox_classes'],
                                                    metric_name='cialg_soft_dice', feature_map_dimensions=(regions_height, regions_width),
                                                    bbox_format=bbox_format, condition_function=_cond_func)
            else:
        ...
        if in_val:
            if skip_nms:
                ...
                if use_soft_dice_metric:
                    append_metric_name(train_metrics_to_merge, val_metrics_to_merge, metrics_to_print, 'cialg_soft_dice', train=False)
            else:
                append_metric_name(train_metrics_to_merge, val_metrics_to_merge, metrics_to_print, 'cialg_bbox_iou', train=False)

    if use_mscxr_for_train or use_mscxr_for_val:
        def _cond_func(x):
            return x['dataset_name'] == 'mscxr'
                ...
                if skip_nms:
                    if use_cnr_metric:
                        attach_condition_aware_bbox_cnr(validator_engine, field_names=['pred_bbox_probs', 'bbox_coords'],
                                                ...
                                                bbox_format=bbox_format, condition_function=_cond_func)
                    if use_soft_dice_metric:
                        attach_condition_aware_bbox_soft_dice(validator_engine, field_names=['pred_bbox_probs', 'bbox_coords'],
                                                metric_name='mscxr_soft_dice', feature_map_dimensions=(regions_height, regions_width),
                                                bbox_format=bbox_format, condition_function=_cond_func)
                else:
        ...
        if in_val:
            if skip_nms:
                ...
                if use_soft_dice_metric:
                    append_metric_name(train_metrics_to_merge, val_metrics_to_merge, metrics_to_print, 'mscxr_soft_dice', train=False)
            else:
    ...
    if use_vinbig_for_train or use_vinbig_for_test:
        def _cond_func_pg(x):
            return x['dataset_name'] == 'vinbig-pg'
        def _cond_func_cl(x):
            return x['dataset_name'] == 'vinbig-cl'
        ...
        if in_train:
            if vinbig_task_mode != VinBigPhraseTaskMode.GROUNDING.value:
                attach_condition_aware_loss(trainer_engine, 'phrase_classifier_loss', _cond_func_cl, 'vbg_phrcls_loss')
                attach_condition_aware_class_averaged_prc_auc(trainer_engine, 'pred_probs', 'gt_labels', None, 'vbg_prc_auc', _cond_func_cl)
            if vinbig_task_mode != VinBigPhraseTaskMode.CLASSIFICATION.value:
                attach_condition_aware_loss(trainer_engine, 'visual_grounding_confidence_loss', _cond_func_pg, 'vbg_vgconf_loss')
                attach_condition_aware_loss(trainer_engine,'visual_grounding_bbox_loss', _cond_func_pg, 'vbg_vgbbox_loss')
        if in_val:
            if vinbig_task_mode != VinBigPhraseTaskMode.GROUNDING.value:
                attach_condition_aware_class_averaged_prc_auc(validator_engine, 'pred_probs', 'gt_labels', None, 'vbg_prc_auc', _cond_func_cl)
            if vinbig_task_mode != VinBigPhraseTaskMode.CLASSIFICATION.value:
                if skip_nms:
                    if use_cnr_metric:
                        attach_condition_aware_bbox_cnr(validator_engine, field_names=['pred_bbox_probs', 'bbox_coords'],
                                                metric_name='vbg_cnr', feature_map_dimensions=(regions_height, regions_width),
                                                bbox_format=bbox_format, condition_function=_cond_func_pg)
                    if use_soft_dice_metric:
                        attach_condition_aware_bbox_soft_dice(validator_engine, field_names=['pred_bbox_probs', 'bbox_coords'],
                                                metric_name='vbg_soft_dice', feature_map_dimensions=(regions_height, regions_width),
                                                bbox_format=bbox_format, condition_function=_cond_func_pg)
                else:
                    attach_condition_aware_bbox_iou_class_agnostic(validator_engine,
                                                        field_names=['predicted_bboxes', 'bbox_coords'],
                                                        metric_name='vbg_bbox_iou', bbox_format=bbox_format,
                                                        condition_function=_cond_func_pg)
        # for logging
        if in_train:
            if vinbig_task_mode != VinBigPhraseTaskMode.GROUNDING.value:
        ...
        if in_val:
            if vinbig_task_mode != VinBigPhraseTaskMode.CLASSIFICATION.value:
                if skip_nms:
                    ...
                    if use_soft_dice_metric:
                        append_metric_name(train_metrics_to_merge, val_metrics_to_merge, metrics_to_print, 'vbg_soft_dice', train=False)
                else:
    ...
    if use_padchest_gr_for_train or use_padchest_gr_for_val:
        def _cond_func(x):
            return x['dataset_name'] == 'padchest_gr'
        ...
        if in_val:
            if skip_nms:
                ...
                if use_soft_dice_metric:
                    attach_condition_aware_bbox_soft_dice(validator_engine, field_names=['pred_bbox_probs', 'bbox_coords'],
                                                    metric_name='pchstgr_soft_dice', feature_map_dimensions=(regions_height, regions_width),
                                                    bbox_format=bbox_format, condition_function=_cond_func)
            else:
                attach_condition_aware_bbox_iou_class_agnostic(validator_engine,
                                                    field_names=['predicted_bboxes', 'bbox_coords'],
        ...
        if in_val:
            if skip_nms:
                ...
                if use_soft_dice_metric:
                    append_metric_name(train_metrics_to_merge, val_metrics_to_merge, metrics_to_print, 'pchstgr_soft_dice', train=False)
            else:
                append_metric_name(train_metrics_to_merge, val_metrics_to_merge, metrics_to_print, 'pchstgr_bbox_iou', train=False)

    if use_chexlocalize:
        def _cond_func(x):
            return x['dataset_name'] == 'cl'
    ...
    if use_cxrlt2024_custom_labels:
        def _cond_func(x):
            return x['dataset_name'] == 'cxrlt2024c'
        # Train
        if use_attention_regularization_loss:
    ...
    if use_cxrlt2024_official_labels:
        def _cond_func(x):
            return x['dataset_name'] == 'cxrlt2024o'
        # Train
        if use_attention_regularization_loss:
    ...
    if len(train_metrics_to_merge) > 0:
        ...
        def score_fn(*unused_args):
            return merge_metrics_fn(trainer_engine.state.metrics, validator_engine.state.metrics)
    else:
        merge_metrics_fn = get_merge_metrics_fn(train_metrics_to_merge, val_metrics_to_merge, _METRIC_WEIGHTS, 0, 1, _metric_getter)
        def score_fn(*unused_args):
            return merge_metrics_fn(validator_engine.state.metrics)
    ...
    run_common_boilerplate_code_and_start_training(
        ...
        build_custom_checkpoint_folder_path=lambda: get_checkpoint_folder_path(experiment_name=experiment_name),
        ...
        wandb_config=experiment_kwargs['wandb_config'],
        device=device,
        ...
        use_determinism_during_validation=False, # Avoid determinism during validation to avoid bugs with CuBLAS
    )
...
def train_from_scratch(
    ...
    chest_imagenome_train_max_size,
    chest_imagenome_val_max_size,
    padchest_gr_val_max_size,
    vinbig_val_max_size,
    ...
    experiment_kwargs = dict(
        ...
        use_cnr_metric=use_cnr_metric,
        use_soft_dice_metric=use_soft_dice_metric,
    )
    ...
    if use_mimiccxr:
        ...
        mimiccxr_trainer_kwargs = dict(
            ...
            chest_imagenome_val_max_size=chest_imagenome_val_max_size,
            chest_imagenome_train_max_size=chest_imagenome_train_max_size,
    ...
    if use_vinbig:
        vinbig_trainer_kwargs = dict(
            ...
            bbox_format=bbox_format,
            val_max_size=vinbig_val_max_size,
        )
    else:
    ...
    if use_padchest_gr:
        padchestgr_trainer_kwargs = dict(
            ...
            val_max_size=padchest_gr_val_max_size,
        )
    else:
...
def resume_training(
    ...
    **unused_kwargs,
    ...
    assert os.path.exists(checkpoint_folder_path), f'Checkpoint folder path does not exist: {checkpoint_folder_path}'
    ...
    experiment_kwargs = metadata['experiment_kwargs']
...
def main(debug=False, no_resume=False):
    inspect_available_ram()
    args = parse_args()
    args = parsed_args_to_dict(args)
    
    checkpoint_folder_paths = find_existing_checkpoint_folder_paths(args['experiment_name'])
    if len(checkpoint_folder_paths) > 0: # resume training from the newest checkpoint
        logger.info(f'Found {len(checkpoint_folder_paths)} existing checkpoint folder paths for experiment'
                    f' {args["experiment_name"]}: {checkpoint_folder_paths}')
        if args['checkpoint_folder_path'] is None:
            logger.info(f'Resuming training from the newest checkpoint: {checkpoint_folder_paths[0]}')
            args['checkpoint_folder_path'] = checkpoint_folder_paths[0]
        else:
            if args['checkpoint_folder_path'] not in checkpoint_folder_paths:
                raise ValueError(f'Checkpoint folder path provided in the arguments: {args["checkpoint_folder_path"]}'
                                 f' does not exist in the existing checkpoint folder paths for experiment'
                                 f' {args["experiment_name"]}: {checkpoint_folder_paths}')
            logger.info(f'Resuming training from the checkpoint provided in the arguments: {args["checkpoint_folder_path"]}')
            args['checkpoint_folder_path'] = checkpoint_folder_paths[checkpoint_folder_paths.index(args['checkpoint_folder_path'])]
    elif args['checkpoint_folder_path'] is not None:
        raise ValueError(f'Checkpoint folder path provided in the arguments: {args["checkpoint_folder_path"]}'
                         f' does not exist in the existing checkpoint folder paths for experiment'
                         f' {args["experiment_name"]}: {checkpoint_folder_paths}')
    
    if args['checkpoint_folder_path'] is not None and not no_resume:
        return resume_training(**args, debug=debug)
    else:
        del args['checkpoint_folder_path']
        del args['override_lr']
        return train_from_scratch(**args, debug=debug)
