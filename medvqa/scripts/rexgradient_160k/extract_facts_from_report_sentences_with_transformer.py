...
def extract_facts(args):
    ...
    for filepath in [
        ...
        logger.info(f"Loaded {len(df)} reports from {filepath}")
        for findings in tqdm(df['Findings'], desc="Collecting sentences from findings"):
            if findings:
                findings = findings.strip()
                for s in sent_tokenize(findings):
                    if s not in already_parsed_sentences:
                        sentences_to_parse.add(s)
        for impression in tqdm(df['Impression'], desc="Collecting sentences from impression"):
            if impression:
    ...
    save_dir = os.path.join(REXGRADIENT_160K_LARGE_FAST_CACHE_DIR, "huggingface")
...
def integrate_facts(args):
    ...
    # Integrate facts into the ReXGradient-160K reports
    final_reports = []
    for filepath, split in [
        (REXGRADIENT_160K_VAL_METADATA_JSON_PATH, 'val'),
        (REXGRADIENT_160K_TEST_METADATA_JSON_PATH, 'test'),
        (REXGRADIENT_160K_TRAIN_METADATA_JSON_PATH, 'train'),
    ]:
        reports = load_json(filepath)
        for key, value in tqdm(reports.items(), desc=f"Integrating facts into {split} reports"):
            report = {
                'key': key,
                'split': split,
                'facts_from_findings': [],
    ...
    output_path = os.path.join(REXGRADIENT_160K_LARGE_FAST_CACHE_DIR, f"rexgradient-160k_reports_with_facts_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract facts from sentences in ReXgradient-160K reports using a seq2seq model.",
    )
    ...
    extract_facts_parser = suparsers.add_parser(
        'extract_facts',
        help="Extract facts from sentences in ReXGradient-160K reports using a seq2seq model.",
    )
    ...
    integrate_facts_parser = suparsers.add_parser(
        'integrate_facts',
        help="Integrate extracted facts into ReXGradient-160K reports.",
    )
