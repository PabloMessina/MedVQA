...
INSTRUCTIONS = """Given a premise from a Chest X-ray report, output an entailment, a contradiction and a neutral sentence.
The entailment must strictly follow logically from the premise. The neutral can optionally be about a different chest X-ray observation.
The output format must be a JSON as follows:
{
"e": ...,
"c": ...,
"n": ...
...
_JSON_FORMAT_REGEX = re.compile(r'^\{\s*"e":\s*"(.*)",\s*"c":\s*"(.*)",\s*"n":\s*"(.*)"\s*\}$')
...
def parse_openai_model_output(text):
    ...
    for key in ["e", "c", "n"]:
        assert key in data, f"Could not parse output: {text}"
        assert isinstance(data[key], str), f"Could not parse output: {text}"
        assert len(data[key]) > 0, f"Could not parse output: {text}"
    return data
...
if __name__ == '__main__':
    ...
    processed_sentences_save_filepath = os.path.join(MIMICCXR_FAST_CACHE_DIR, "openai", f"{args.openai_model_name}_nli_examples{args.alias}.jsonl")
